...
import csv
import uuid
from .time_utils import ist_time, IST
import io
import hashlib
...
def generate_csv_report(detections: List[dict], filename: str = None) -> str:
    ...
    if not filename:
        filename = f"GUIVIN_Detection_Report_{datetime.now().strftime('%Y%m%d_%H%M%S') + '_' + uuid.uuid4().hex[:8]}.csv"
    ...
    fieldnames = [
        ...
        "Alert ID", "Local Ledger Block", "Evidence Hash (SHA-256)", "Origin"
    ]
    ...
    with open(filepath, "w", newline="", encoding="utf-8") as f:
        ...
        for d in detections:
            ...
            row = {
                ...
                "Local Ledger Block": d.get("block_id", ""),
                "Evidence Hash (SHA-256)": d.get("clip_hash", ""),
                "Origin": d.get("origin", "SIMULATED"),
            }
...
def generate_pdf_report(alerts: List[dict], camera_map: dict,
    ...
    if not filename:
        filename = f"GUIVIN_Incident_Report_{datetime.now().strftime('%Y%m%d_%H%M%S') + '_' + uuid.uuid4().hex[:8]}.pdf"
    ...
    summary_data = [
        ...
        ["Local Ledger Entries", str(sum(1 for a in alerts if a.get("block_id")))],
    ]
    ...
    # Alert table
    story.append(Paragraph("Alert Log (IST) - Local Evidence Ledger", section_style))
    headers = ["Alert ID", "Camera", "Time", "Severity", "Plate", "Risk", "Block #", "Hash (first 16)"]
    rows = [headers]
    for a in alerts[:50]:   # max 50 rows in PDF
        ts = a.get("timestamp", "")
        try:
            ts_fmt = ist_time(ts).strftime("%H:%M:%S")
        except Exception:
            ts_fmt = ts
        rows.append([
            a.get("id", "")[:12],
    ...
    col_widths = [24*mm, 22*mm, 17*mm, 17*mm, 25*mm, 10*mm, 13*mm, 27*mm]
    ...
    story.append(Paragraph(
        ...
        "Captured evidence is hashed into a local tamper-evident ledger. "
        "Metadata-only and legacy events are not verified evidence. No external trust anchor or legal guarantee is provided. "
        "Prototype report; review source events before use.",
...
def get_detections_for_report(alerts: List[dict], camera_map: dict) -> List[dict]:
    ...
    for a in alerts:
        ...
        rows.append({
            ...
            "plate_confidence": a.get("plate_confidence", 0),
            "watchlist_match": "YES" if a.get("alert_type") == "WATCHLIST_MATCH" else "NO",
            "watchlist_source": a.get("watchlist_source", ""),
